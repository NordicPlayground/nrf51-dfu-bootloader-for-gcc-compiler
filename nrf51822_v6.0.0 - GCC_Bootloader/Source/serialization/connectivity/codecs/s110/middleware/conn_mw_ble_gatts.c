                                     ...
                                     uint32_t * const      p_tx_buf_len)
{
    ...
    uint16_t   offset;
    uint8_t    attr_val[BLE_GATTS_VAR_ATTR_LEN_MAX];
    uint8_t *  p_attr_val = attr_val;
    uint16_t   len        = sizeof (attr_val);
    uint16_t * p_len      = &len;

    uint32_t err_code = NRF_SUCCESS;
    uint32_t sd_err_code;

    err_code = ble_gatts_value_set_req_dec(p_rx_buf, rx_buf_len, &attr_handle, &offset, &p_len,
                                           &p_attr_val);
    SER_ASSERT(err_code == NRF_SUCCESS, err_code);

    sd_err_code = sd_ble_gatts_value_set(attr_handle, offset, p_len, p_attr_val);

    err_code = ble_gatts_value_set_rsp_enc(sd_err_code, p_tx_buf, p_tx_buf_len, len);
    SER_ASSERT(err_code == NRF_SUCCESS, err_code);
                                     ...
                                     uint32_t * const      p_tx_buf_len)
{
    ...
    err_code = ble_gatts_value_get_req_dec(p_rx_buf, rx_buf_len, &attr_handle, &offset, &p_len,
                                           &p_attr_val);
    SER_ASSERT(err_code == NRF_SUCCESS, err_code);

    sd_err_code = sd_ble_gatts_value_get(attr_handle, offset, p_len, p_attr_val);

    err_code = ble_gatts_value_get_rsp_enc(sd_err_code, p_tx_buf, p_tx_buf_len, p_attr_val, p_len);
    SER_ASSERT(err_code == NRF_SUCCESS, err_code);
                                        ...
                                        uint32_t * const      p_tx_buf_len)
{
    ...
    sd_err_code = sd_ble_gatts_sys_attr_set(conn_handle, p_sys_attr, sys_attr_len);
                                        ...
                                        uint32_t * const      p_tx_buf_len)
{
    ...
    sd_err_code = sd_ble_gatts_sys_attr_get(conn_handle, p_sys_attr, p_sys_attr_len);
