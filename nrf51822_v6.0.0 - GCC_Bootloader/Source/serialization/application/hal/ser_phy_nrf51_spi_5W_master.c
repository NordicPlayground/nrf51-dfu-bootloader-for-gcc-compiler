...
_static volatile bool m_slaveReadyFlag   = false;
_static volatile bool m_slaveRequestFlag = false;
...
//For 5W slave is considered to be always READY
        ...
        (NRF_GPIOTE->INTENSET & GPIOTE_INTENSET_IN1_Msk))
    {
        ...
        {
            m_slaveReadyFlag = false;
        }

        if (m_slaveReadyFlag)
        {
            DEBUG_EVT_SPI_MASTER_RAW_READY_EDGE(0);
        }
        else
        {
            else
                DEBUG_EVT_SPI_MASTER_RAW_READY_EDGE(1);
        }
    }
#endif
        ...
        (NRF_GPIOTE->INTENSET & GPIOTE_INTENSET_IN0_Msk))
    {
        ...
        {
            m_slaveRequestFlag = true;
            m_pend_req_flag    = true;
        }
        else
        {
            m_slaveRequestFlag = false;
        }

        if (m_slaveRequestFlag)
        {
            DEBUG_EVT_SPI_MASTER_RAW_REQUEST_EDGE(0);
        }
        else
        {
            DEBUG_EVT_SPI_MASTER_RAW_REQUEST_EDGE(1);
        }
    }
...
static __INLINE void ser_phy_init_gpiote()
{
    ...
    m_slaveRequestFlag = !(nrf_gpio_pin_read(SER_PHY_SPI_MASTER_PIN_SLAVE_REQUEST));
    nrf_gpiote_event_config(0, SER_PHY_SPI_MASTER_PIN_SLAVE_REQUEST, NRF_GPIOTE_POLARITY_TOGGLE);

#ifdef _SPI_5W_
    m_slaveReadyFlag     = true;
    NRF_GPIOTE->INTENSET = GPIOTE_INTENSET_IN0_Msk;
#else
    m_slaveReadyFlag = !(nrf_gpio_pin_read(SER_PHY_SPI_MASTER_PIN_SLAVE_READY));
    nrf_gpiote_event_config(1, SER_PHY_SPI_MASTER_PIN_SLAVE_READY, NRF_GPIOTE_POLARITY_TOGGLE);
    //Enable interrupt for NRF_GPIOTE->EVENTS_IN[0] && NRF_GPIOTE->EVENTS_IN[1]
